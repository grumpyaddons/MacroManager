...
local spellCache = {}
MacroMicro.spellCache = spellCache
...
function spellCache.Build()
  if not cache  then
    error("spellCache has not been loaded. Call MacroMicro.spellCache.Load(...) first.")
  end
  ...
  local co = coroutine.create(function()
    ...
    -- Updates the icon cache with whatever icons MacroMicro core has actually used.
    -- This helps keep name<->icon matches relevant.
    for name, icons in pairs(MacroMicroSaved.dynamicIconCache) do
      if MacroMicroSaved.dynamicIconCache[name] then
        for spellId, icon in pairs(MacroMicroSaved.dynamicIconCache[name]) do
  ...
  Private.dynFrame:AddAction("spellCache", co)
end
...
function spellCache.Load(data)
  ...
  local version = MacroMicro.versionString
...
function spellCache.CorrectAuraName(input)
  ...
  local spellId = MacroMicro.SafeToNumber(input);
  if(spellId) then
